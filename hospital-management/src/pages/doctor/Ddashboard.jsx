...
const Ddashboard = () => {
  const { pdashboardState, setPdashboardState, backend_url } = useContext(AuthContext);
  ...
  const grantAppointment = async (tappointmentID, tappointmentTime, tcost) => {
    console.log(tappointmentID)
    try {
      const response = await fetch(`${backend_url}/appointments/doctor/${doctorId}/grant`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({ appointmentID: parseInt(tappointmentID), appointmentTime: tappointmentTime, cost: tcost }),
      });
